...
public function updateLandPostFund(Request $request)
{
    $request->validate([
        'form_id' => 'required',
        'area_benefited_postfunding' => 'required|string|max:255',
    ]);

    // Update LandForm
    LandForm::where('form_id', $request->form_id)
        ->update(['area_benefited_postfunding' => $request->area_benefited_postfunding]);
...
public function updatePondPostFund(Request $request)
{
    ...
    PondForm::where('form_id', $request->form_id)->update([
        ...
        'vol_pf' => $vol_pf,
        'area_benefited_postfunding' => $request->area_benefited_postfunding,
    ]);
...
public function updateFarmerDetails(Request $request)
{
    ...
    $form->update([
        ...
        'toilet_avail' => $request->toilet_avail,
...
public function updatePlantForm(Request $request)
{
    ...
    if ($plant) {
        $plant->update([
            ...
            'plantaions' => $request->pl_type,
            'sf_number' => $request->pl_sf_no,
            'soil_type' => $request->pl_soil_type,
            'land_to_benefit' => $request->pl_land_benefit,
            ...
            'area_benefited_by_proposal' => $request->pl_area_benefit,
